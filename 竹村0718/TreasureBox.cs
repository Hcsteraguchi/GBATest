...
public class TreasureBox : MonoBehaviour
{
    ...
    [Header("カメラについているUIを入れる")] [SerializeField] private GameObject _camera = default;
    [Header("この宝箱に入れるアイテムを格納するリスト")]
    [SerializeField]
    private List<Inventory.WeaponSelect> _itemBoxList =
        new List<Inventory.WeaponSelect>();
    [Header("今選ばれているアイテム")] [SerializeField] public int _item = 0;

    //宝箱開封中かどうかの判定
    private bool _isopenBox = default;

    //必要な他クラス
    Inventory _inventorycs;
    PlayerMove_KM _playerMove;
    InventUItest _inventUItest;
    private void Start()
    {
        ...
        _inventUItest = _camera.GetComponent<InventUItest>();
    }
    ...
    private void TreasureSet()
    {
        ...
        for (int i = 0; i <= _inventorycs._getMaxIndex; i++)
        {
       ...
       */
        ...
        _inventUItest.Tresure(_itemBoxList);
    }
    private void WeaponGet()
    {
        ...
        if (Input.GetButtonDown("Attack"))
        {
            ...
            _inventUItest.AddItem(_itemBoxList[_item]);
